    ...
    for t_idx in range(predicted_time_steps)[:1]:
        for group in model_ds.keys():
            ...
            for feature_name in pred.feature_names_to_idx.keys():
                ...
                level, name, tol = grib_features.loc[feature_name, ['level', 'name','typeOfLevel']]
...
def get_output_filename(saving_settings: GribSavingSettings, sample: Any, leadtime: float) -> str:
    identifiers = []
    ...
    for param in params:
        ...
        for ft_idx, ftname in enumerate(trial_names):
            if ftname in pred_feature_names:
                ...
                list_features.append(pd.DataFrame(
                    {"feature_name" : [ftname],
                    "level" : [level],
                    "name" : [name],
                    "typeOfLevel" : [tol]}, index=[ftname]
                ))
...
def get_grib_groups(grib_features : pd.DataFrame) -> dict[str : dict]:
    ...
    grib_groups = {
        "surface": {
            "cfVarName":grib_features["name"].loc[(grib_features["typeOfLevel"]=="surface")].tolist(),
            "typeOfLevel": "surface",
        },
    }
    ...
    for tol in typesOflevel:
        if tol != "surface":
            levels_tol = grib_features["level"].loc[(grib_features["typeOfLevel"]==tol)]
...
def make_nan_mask(
    ...
    try:
        ...
        raise NotImplementedError(f"The dataset {infer_dataset} has no grid attribute, cannot write grib.")
    ...
    if (template_dataset.latitude.values != infer_dataset.grid.lat[0, :]) or (
        ...
        try:
            ...
            latmin, latmax, longmin, longmax = latmin.item(), latmax.item(), longmin.item(), longmax.item()
        except ValueError:
            raise ValueError(f"Lat/Lon dims of the {infer_dataset} do not fit in template grid, cannot write grib.")
