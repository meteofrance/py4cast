...
def saveNamedTensorToGrib(
    pred: NamedTensor, ds : DatasetABC, sample: Any, date: dt.datetime, saving_settings: dict
) -> None:
    """
    ...
    if hasattr(ds,'grib_keys_converter'):
        ...
        print("Found no custom grib keys converter implemented for dataset, resorting to default grib keys getter")
        params = ds.params
    ...
    for t_idx in range(predicted_time_steps)[:1]:
        ...
        if (target_ds.latitude.values != ds.grid.lat[0,:]) or (target_ds.longitude.values !=ds.grid.lon[:,0]):
            nanmask = np.empty((len(target_ds.latitude),len(target_ds.longitude)))
            nanmask[:] = np.nan
            latmin, latmax = np.where(np.round(target_ds.latitude.values,5)==round(ds.grid.lat.min(),5))[0].item(), np.where(np.round(target_ds.latitude.values,5)==round(ds.grid.lat.max(),5))[0].item()
            longmin, longmax = np.where(np.round(target_ds.longitude.values,5)==round(ds.grid.lon.min(),5))[0].item(), np.where(np.round(target_ds.longitude.values,5)==round(ds.grid.lon.max(),5))[0].item()
        ...
        target_ds["valid_time"] = np.datetime64(date) + np.timedelta64(nanosecond_term,'ns')
    ...
    for param in params:
        ...
        for ft_idx, ftname in enumerate(trial_names):
            ...
            if ftname in pred_feature_names:
                level, name, typeoflevel = param.levels[ft_idx], param.name, param.level_type
                grib_keys[ftname] = {"level": level, "name": name, "typeOfLevel" : typeoflevel}
...
def get_grib_groups(grib_keys: dict, typesOflevel: dict) -> dict:
    ...
    grib_groups = {
        "surface" : {
                     "cfVarName" :[grib_keys[ftname]["name"]
                        for ftname in grib_keys.keys() if grib_keys[ftname]["typeOfLevel"]=='surface'],
                     "typeOfLevel" : "surface"
        },
        }
