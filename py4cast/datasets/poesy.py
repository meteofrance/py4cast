...
scratch_path = Path("/scratch/shared/poesy")
...
def poesy_forecast_namer(date: dt.datetime, shortname, **kwargs):
    ...
    """
    return f"{date.strftime('%Y-%m-%dT%H:%M:%SZ')}_{shortname}_lt1-45crop.npy"
...
# Define static ttirbutes to add -> see Grid class in smeagol.py
...
class Grid:
    ...
    def geopotential(self) -> np.array:
        # To Do : a changer (ici on ne prend pas en compte qu'on travail avec les donnes crop)
        ds = np.load(self.grid_name)
...
class Param:
    ...
    def load_data(self, date: dt.datetime, term: List, file_date: int = 21):
        """
        date : Date du fichier
        term : Position des echeances dans le fichier.
        """
...
class PoesyDataset(DatasetABC, Dataset):
    ...
    def get_param_tensor(
        ...
        array = param.load_data(date, terms)
...
if __name__ == "__main__":
    ...
    train_ds, val_ds, test_ds = PoesyDataset.from_json(path_config, 2, 1, 5)
    dataset_info = train_ds.dataset_info
    print(train_ds.__getitem__(0))
    print(train_ds.statics)
    dataset_info.summary()
    print(train_ds.meshgrid.shape)
