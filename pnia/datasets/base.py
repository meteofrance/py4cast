...
class AbstractDataset(ABC):
    ...
    def torch_save(self, data, path):
        """
        Saving files to be writeable by anyone 
        """ 
    ...
    def compute_parameters_stats(self): 
        ...
        if weather_names == forcing_names: 
            raise ValueError(f"You may have a problem with your name : your forcing and weather variable are the same and are {forcing_names}.")
        ...
        for init_batch, target_batch, _, forcing_batch in tqdm(self.loader):
            ...
            flux_batch = forcing_batch[:, :, :, :forced_len]  # Removing date. It is cleaner than postponing forcing is the first variable (as it allows multiple forcing). 
            flux_means.append(torch.mean(flux_batch, dim=(1, 2)))   # (N_batch, d_forcing,)
            flux_squares.append(torch.mean(flux_batch**2, dim=(1, 2)))   # (N_batch, d_forcing,)
        ...
        flux_second_moment = torch.mean(torch.cat(flux_squares, dim=0), dim=0)  # # (d_forcing)
        ...
        for i,name in enumerate(self.shortnames("input_output")): 
            store_d[name] = {"mean":mean[i], "std": std[i], "min": mini[i],"max":maxi[i]}
        
        # Storing flux statistics 
        for i,name in enumerate(self.shortnames("forcing")): 
            store_d[name] = {"mean":flux_mean[i], "std": flux_std[i], "min":flux_min[i], "max":flux_max[i]}
        self.torch_save(store_d, self.cache_dir / "parameters_stats.pt")
    ...
    def compute_timestep_stats(self):
        ...
        self.torch_save(store_d, self.cache_dir / "diff_stats.pt")
    ...
    def load_static_data(self, device="cpu")->Statics:
        ...
        diff_stats = self.load_file("diff_stats.pt") # (d_f,)
        step_diff_mean = []
        step_diff_std = []
        ...
        stats = self.load_file("parameters_stats.pt")
        data_mean = []
        data_std = []
        for name in self.shortnames("input_output"): 
            if stats[name]["std"] == 0: 
