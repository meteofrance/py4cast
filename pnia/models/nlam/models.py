...
class BaseGraphModel(ARModel):
    ...
    def __init__(self, hp: HyperParam):
        ...
        self.grid_embedder = utils.make_mlp(
            [grid_dim] + self.mlp_blueprint_end, checkpoint=hp.graph.checkpoint
        )
        self.g2m_embedder = utils.make_mlp(
            [g2m_dim] + self.mlp_blueprint_end, checkpoint=hp.graph.checkpoint
        )
        self.m2g_embedder = utils.make_mlp(
            [m2g_dim] + self.mlp_blueprint_end, checkpoint=hp.graph.checkpoint
        )
        ...
        self.encoding_grid_mlp = utils.make_mlp(
            [hp.graph.hidden_dim] + self.mlp_blueprint_end,
            checkpoint=hp.graph.checkpoint,
        )
...
class BaseHiGraphModel(BaseGraphModel):
    ...
    def __init__(self, hp):
        ...
        self.mesh_embedders = nn.ModuleList(
            [
                utils.make_mlp(
                    [mesh_dim] + self.mlp_blueprint_end, checkpoint=hp.graph.checkpoint
                )
                for _ in range(self.N_levels)
            ]
        )
        if hp.graph.offload:
            self.mesh_embedders = offload(self.mesh_embedders)
        ...
        if hp.graph.offload:
            self.mesh_same_embedders = offload(self.mesh_same_embedders)
        ...
        if hp.graph.offload:
            self.mesh_up_embedders = offload(self.mesh_up_embedders)
        ...
        if hp.graph.offload:
            self.mesh_down_embedders = offload(self.mesh_down_embedders)
        ...
        if hp.graph.offload:
            self.mesh_init_gnns = offload(self.mesh_init_gnns)
        ...
        if hp.graph.offload:
            self.mesh_read_gnns = offload(self.mesh_read_gnns)
...
class GraphLAM(BaseGraphModel):
    ...
    def __init__(self, hp):
        ...
        self.m2m_embedder = utils.make_mlp(
            [m2m_dim] + self.mlp_blueprint_end, checkpoint=hp.graph.checkpoint
        )
...
class HiLAM(BaseHiGraphModel):
    ...
    def __init__(self, hp):
        ...
        self.mesh_down_gnns = nn.ModuleList(
            [self.make_down_gnns(hp) for _ in range(hp.graph.processor_layers)]
        )  # Nested lists (proc_steps, N_levels-1)

        self.mesh_down_same_gnns = nn.ModuleList(
            [self.make_same_gnns(hp) for _ in range(hp.graph.processor_layers)]
        )  # Nested lists (proc_steps, N_levels)

        # Make up GNNs, both for up edges and same level
        self.mesh_up_gnns = nn.ModuleList(
            [self.make_up_gnns(hp) for _ in range(hp.graph.processor_layers)]
        )  # Nested lists (proc_steps, N_levels-1)
